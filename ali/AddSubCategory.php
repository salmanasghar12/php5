...
<?php
//Getting all Existing Categories from Category table
mysql_select_db($database_seoExchangeDbConnection, $seoExchangeDbConnection);
$query_RecordsetCategories = "SELECT DISTINCT category.name FROM category";
...
$totalRows_RecordsetCategories = mysql_num_rows($RecordsetCategories);
	...
	mysql_select_db($database_seoExchangeDbConnection, $seoExchangeDbConnection);
	$queryGetSubcatId = sprintf("SELECT id FROM subcategory ORDER BY id DESC LIMIT 1");
	...
	//now we will add into category 
	 
	mysql_select_db($database_seoExchangeDbConnection, $seoExchangeDbConnection);
	$queryAddIntoCategory = sprintf("INSERT INTO category (name, subcategory) VALUES (%s, %s)", GetSQLValueString($_POST['Category'], "text"), GetSQLValueString($subCategoryId, "int"));
	$recordSetAddIntoCategory = mysql_query($queryAddIntoCategory, $seoExchangeDbConnection) or die(mysql_error());
	}
...
<div id="sidebar">
  ...
  <div id="content">
    <div class="panel">
      <div class="panelBody">
      <?php if(isset($recordSetAddIntoCategory)){echo "A new Sub-Category Added.";}?>
        <h1>Add a Sub-Category:</h1>
        <p>To Add a New Sub-Category, please choose one existing Category from Category List.</p>
        <form id="formCategory" method="post" action="<?php echo $_SERVER['PHP_SELF']?>"><table width="716">
          <tr>
            <td width="133" height="39" class="linksStyle">Choose a Category :</td>
            <td width="567"><span id="spryselect1">
...
?>
          ...
          <tr>
            <td height="74" class="linksStyle">Sub-Category Name:</td>
